...
- **Items (Resources)**: `GET /api/items` and `GET /api/items/{id}` surface ReqRes `/unknown` resources, treated as warehouse items. Local `POST /api/items` and `PUT /api/items/{id}` endpoints let you add and edit resource details (including a locally managed **quantity**) in memory. Results are built from the shared cache so newly added or edited items remain visible even when the upstream call succeeds with stale data.
