...
- **Resources**: Lists ReqRes `unknown` records via `/api/items`, lets you add local resources (POST), and edit existing ones (PUT) so tooling can be managed alongside associates. Quantity is tracked locally for each item so edits remain visible even when ReqRes returns the original values.
